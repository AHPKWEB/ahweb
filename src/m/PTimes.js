let timings={}, nextPrayer="", nextTime=null;

// English names
function englishName(n){
return{Fajr:"Fajr",Dhuhr:"Dhuhr",Asr:"Asr",Maghrib:"Maghrib",Isha:"Isha"}[n];
}

// 24h -> 12h AM/PM
function to12(time){
let [h,m]=time.split(':'); h=parseInt(h);
let ampm = h>=12?"PM":"AM";
h = h%12; h = h? h:12;
return h+":"+m+" "+ampm;
}

// Load today's prayer times
async function loadToday(){
const res = await fetch(`https://api.aladhan.com/v1/timingsByCity?city=${city}&country=${country}&method=2&school=${school}`);
const data = await res.json();

// Hijri date
document.getElementById("hijri").innerHTML=
data.data.date.hijri.weekday.en+" "+
data.data.date.hijri.day+" "+
data.data.date.hijri.month.en+" "+
data.data.date.hijri.year+" Hijri";

// Timings
const t = data.data.timings;
timings={Fajr:t.Fajr,Dhuhr:t.Dhuhr,Asr:t.Asr,Maghrib:t.Maghrib,Isha:t.Isha};

renderTimes();
setNext();
startCountdown();
}

// Render prayer times
function renderTimes(){
let html="";
for(let p in timings){
html+=`<div class="prayer" id="p_${p}"><span>${englishName(p)}</span><span>${to12(timings[p])}</span></div>`;
}
document.getElementById("times").innerHTML = html;
}

// Set next prayer
function setNext(){
const now = new Date();
nextTime = null;
for(let p in timings){
let [h,m] = timings[p].split(':');
let d = new Date(); d.setHours(h,m,0);
if(d>now){ nextPrayer=p; nextTime=d; break; }
}
if(!nextTime){
let [h,m] = timings.Fajr.split(':');
nextTime=new Date(); nextTime.setDate(nextTime.getDate()+1); nextTime.setHours(h,m,0);
nextPrayer="Fajr";
}
document.getElementById("next").innerHTML ='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 481.22 565.28"><defs><style>.cls-1{fill:#c2222a;}</style></defs><g id="Layer_2" data-name="Layer 2"><g id="Layer_1-2" data-name="Layer 1"><path fill="#fff" d="M197.74,235.83c3.38,2,7.22,3.42,10,6,9.63,8.74,5,24.5-8.1,28.6-4.59,1.43-4.59,1.43-4.59,6.23,0,7.81.09,15.63,0,23.44a4.68,4.68,0,0,0,2.13,4.47c5.9,3.86,11.66,8,17.78,11.46,13.51,7.69,27.58,14.46,40.76,22.68a275.7,275.7,0,0,1,66.19,57c14.28,17,19.89,37,19.52,59-.37,22.31-8,41.77-22.65,58.53a3.14,3.14,0,0,0-.28.75h50.91v-3.21c0-17.81-.1-35.63,0-53.45.08-10.1,2.06-19.88,7.53-28.53,5.63-8.91,7.27-18.75,7.3-28.91q.21-66.25.05-132.53a8,8,0,0,0-1.06-3.76c-5.31-9.38-10.77-18.68-16.06-28.08a11.1,11.1,0,0,1-1.35-4.94c-.14-7.18-.06-14.35-.06-21.76H374c.07,1.06.13,2.08.22,3.31H388c.3,0,.15,1.54.16.74,0,7.91.13,108.82-.06,36.73,0-1.39-.94-2.78-1.47-4.16-.07-.18-.26-.31-.4-.47-3.05-3.41-7.35-6.36-8.82-10.36-1.55-4.23-.43-9.45-.45-14.24,0-3.64,0-7.29,0-11.18h6.61c.06,1.22.13,2.43.19,3.81h7.38v-3.41c0-30,.42-60-.19-90-.27-13.52,4-25,11.17-35.84,2.3-3.46,4.9-6.72,7.4-10,2.94-3.91,3-6,0-9.71a46.12,46.12,0,0,1-10.82-34.11,32.75,32.75,0,0,1,7.3-18.18c3.14-4,6.45-7.8,9.67-11.69h.63c2.25,2.44,4.79,4.68,6.68,7.38,3.19,4.56,7,9.05,8.71,14.19,4.95,15.29,1.33,29.31-8.52,41.8-3.48,4.42-3.56,6.26-.14,10.73,2.27,3,4.56,5.94,6.65,9,7.55,11.16,12,23,11.68,37-.6,29.79-.19,59.6-.19,89.4V213h7.39l.19-3.91h6.4c0,4.09,0,8,0,12,0,4.58,1.1,9.61-.44,13.62S449,241.72,445.8,245c-1.5,1.56.05,3.17.05,5.27.13,71.58-2-12.85-2-21.28V212.12H458l.15-3.32h8.08c0,7.5.07,14.79-.06,22.08a10.27,10.27,0,0,1-1.26,4.65c-4.57,8.13-9.17,16.26-14,24.24A22.31,22.31,0,0,0,447.53,272c.22,42.1.18,84.19.3,126.29,0,10.92,1.5,21.45,7.58,31,5.12,8.07,7,17.27,7.11,26.75.14,19.9,0,39.81.11,59.71,0,1,1,2.65,1.53,2.67,1.19,0,3-.44,3.53-1.32,2.17-3.38,3.51-3.89,6.81-1.6a25.23,25.23,0,0,1,4.79,4,7.25,7.25,0,0,1,1.86,4.21c.09,46.06.08-14.88.07,31.17,0,.8-.11,1.59-.18,2.46H0v-3L4.3,554c.06-.81.17-1.7.17-2.58,0-7.3,0-14.59,0-21.88,0-9.26,5.37-14.68,14.73-14.76,8.44-.08,16.87,0,25.31,0h15c-.69-.87-1.13-1.48-1.62-2.05C41.28,493.11,34.34,470.55,36,444.9c1.59-24.3,12.71-43.54,29.28-60.52a318.06,318.06,0,0,1,86.35-62.73c8.61-4.3,16.73-9.58,25-14.47,4-2.36,6.14-5.38,5.76-10.61-.59-8.28-.15-16.64-.15-25.19-5.85-1.29-11.18-3.39-14.4-8.81a18.78,18.78,0,0,1-2.33-11.69c.6-6.41,7.45-13.46,13.58-14.55-1.8,2.79-4.35,5.74-5.78,9.15-2.7,6.39,0,11.9,6.53,14.14a28.19,28.19,0,0,0,18.1.1c7.55-2.44,10-9.09,5.84-16.18-1.64-2.78-4.21-5-6.36-7.48Z"/><path class="cls-1" d="M411.77,165.25v47.22h10.29V165.25Z"/><path class="cls-1" d="M437,212.63V165.27h-3.57v47.36Z"/><path class="cls-1" d="M396.83,165.29v47.24h3.52V165.29Z"/><path d="M203.48,330.06l-.59-.35.85.12Z"/></g></g></svg>' + englishName(nextPrayer);
}
// Play azan
function playAzan(p){ let el=document.getElementById("azan"+p); if(el) el.play(); }

// Countdown + notifications
function startCountdown(){
setInterval(()=>{
const now = new Date();
let diff = nextTime - now;
if(diff<0) setNext();

let totalSeconds = Math.max(0,Math.floor(diff/1000));
let h=Math.floor(totalSeconds/3600);
let m=Math.floor((totalSeconds%3600)/60);
let s=totalSeconds%60;

// Next prayer time in 12h
let nh=nextTime.getHours(), nm=nextTime.getMinutes();
let ampm = nh>=12?"PM":"AM";
let displayHour = nh%12; displayHour=displayHour?displayHour:12;
let nextTimeStr = displayHour+":"+String(nm).padStart(2,'0')+" "+ampm;

// Update header countdown with <span> for dash
document.getElementById("countdown").innerHTML =
  `<span class="time">${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}</span>
   <span class="separator"> — </span>
   <span class="prayer-name">${englishName(nextPrayer)} ${nextTimeStr}</span>`;
  
// Highlight next prayer
document.querySelectorAll(".prayer").forEach(e=>e.classList.remove("active"));
let el = document.getElementById("p_"+nextPrayer); if(el) el.classList.add("active");

// Notification 1 min before
if(totalSeconds===60){ notify(); }

// Play Azan
if(totalSeconds===0){ playAzan(nextPrayer); }

},1000);
}

// Notification
function notify(){
if(Notification.permission==="granted"){
new Notification("Prayer Time",{body:"It's now time for "+englishName(nextPrayer)});
}else if(Notification.permission!=="denied"){
Notification.requestPermission();
}
}

// Monthly timetable
async function loadMonthly(){
const now=new Date();
const res=await fetch(`https://api.aladhan.com/v1/calendarByCity?city=${city}&country=${country}&method=2&school=${school}&month=${now.getMonth()+1}&year=${now.getFullYear()}`);
const data=await res.json();

let html="<table>";
html+="<tr><th>Date</th><th>Fajr</th><th>Dhuhr</th><th>Asr</th><th>Maghrib</th><th>Isha</th><th>Sehri</th><th>Iftar</th></tr>";
data.data.forEach(d=>{
let ramadan = d.date.hijri.month.en==="Ramadan";
html+=`<tr class="${ramadan?'ramadan':''}"><td>${d.date.gregorian.day}</td><td>${to12(d.timings.Fajr)}</td><td>${to12(d.timings.Dhuhr)}</td><td>${to12(d.timings.Asr)}</td><td>${to12(d.timings.Maghrib)}</td><td>${to12(d.timings.Isha)}</td><td>${ramadan?to12(d.timings.Fajr):"-"}</td><td>${ramadan?to12(d.timings.Maghrib):"-"}</td></tr>`;
});
html+="</table>";
document.getElementById("monthly").innerHTML = html;
}

// Toggle monthly table
function toggleMonthly(){
const el=document.getElementById("monthly");
if(el.style.display==="block"){ el.style.display="none"; }
else{ el.style.display="block"; if(!el.innerHTML) loadMonthly(); }
}

// Initialize
loadToday();